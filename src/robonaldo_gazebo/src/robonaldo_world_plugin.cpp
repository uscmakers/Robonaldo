...
public:
  void Load(physics::WorldPtr _parent, sdf::ElementPtr /*_sdf*/)
  {
    ...
    // MUST DO: Get location configs to figure out ball location

    // MUST DO: Publish to ROS node
    // Initialize ros, if it has not already bee initialized.
    if (!ros::isInitialized())
    {
      int argc = 0;
      char **argv = NULL;
      ros::init(argc, argv, "robonaldo_gazebo_world",  // MUST DO: change this node name
    ...
    // MUST DO: Create our ROS node. This acts in a similar manner to the Gazebo node.
  }
private:
  physics::WorldPtr world;
private:
  physics::ModelPtr robo;
private:
